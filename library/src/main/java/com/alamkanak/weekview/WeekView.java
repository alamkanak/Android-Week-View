...
public class WeekView extends View {
    ...
    private void drawEventTitle(WeekViewEvent event, RectF rect, Canvas canvas, float originalTop, float originalLeft) {
        ...
        SpannableStringBuilder eventText = new SpannableStringBuilder();
        if (event.getName() != null) {
            eventText.append(event.getName());
            eventText.setSpan(new StyleSpan(android.graphics.Typeface.BOLD),0,eventText.length(),0);
            eventText.append(' ');
        }
        if (event.getLocation() != null) {
            eventText.append(event.getLocation());
        }

        // Get text dimensions
        StaticLayout textLayout = new StaticLayout(eventText, mEventTextPaint, (int) (rect.right - originalLeft - mEventPadding * 2), Layout.Alignment.ALIGN_NORMAL, 1.0f, 0.0f, false);
        ...
        if (lineHeight < availableHeight && textLayout.getHeight() > rect.height() - mEventPadding * 2) {
            ...
            textLayout = new StaticLayout(TextUtils.ellipsize(eventText, mEventTextPaint, widthAvailable, TextUtils.TruncateAt.END), mEventTextPaint, (int) (rect.right - originalLeft - mEventPadding * 2), Layout.Alignment.ALIGN_NORMAL, 1.0f, 0.0f, false);
        }
        else if (lineHeight >= availableHeight) {
            int width = (int) (rect.right - originalLeft - mEventPadding * 2);
            textLayout = new StaticLayout(TextUtils.ellipsize(eventText, mEventTextPaint, width, TextUtils.TruncateAt.END), mEventTextPaint, width, Layout.Alignment.ALIGN_NORMAL, 1.0f, 1.0f, false);
        }
