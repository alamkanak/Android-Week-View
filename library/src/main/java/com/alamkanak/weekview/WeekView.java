...
public class WeekView extends View {
    ...
    private int mEventMarginHorizontal = 0;
    ...
    private void drawHeaderRowAndEvents(Canvas canvas) {
        ...
        for (int dayNumber = leftDaysWithGaps + 1;
            ...
            if (mNumberOfVisibleDays == 1) {
                startPixel = startPixel + mEventMarginHorizontal;
            }
        ...
        if (mEventRects != null && mEventRects.size() > 0) {
            for (int i = 0; i < mEventRects.size(); i++) {
                if (isSameDay(mEventRects.get(i).event.getStartTime(), date) && !mEventRects.get(i).event.isAllDay()) {
                    ...
                    boolean hasNoOverlaps = (right == startFromPixel + mWidthPerDay);
                    if (mNumberOfVisibleDays == 1 && hasNoOverlaps) {
                        right -= mEventMarginHorizontal * 2;
    ...
    private void drawAllDayEvents(Calendar date, float startFromPixel, Canvas canvas) {
        if (mEventRects != null && mEventRects.size() > 0) {
            for (int i = 0; i < mEventRects.size(); i++) {
                WeekViewEvent event = mEventRects.get(i).event;
        ...
        if (mDateTimeInterpreter == null) {
            mDateTimeInterpreter = new DateTimeInterpreter() {
                ...
                public String interpretDate(Calendar date) {
                    try {
                        SimpleDateFormat sdf = mDayNameLength == LENGTH_SHORT
    ...
    public void setEventMarginHorizontal(int eventMarginHorizontal) {
        this.mEventMarginHorizontal = eventMarginHorizontal;
        invalidate();
    }

    public int getEventMarginHorizontal() {
        return mEventMarginHorizontal;
    }
